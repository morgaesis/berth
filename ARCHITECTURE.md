...
The auto-entry signal for new tabs:

1. `BERTH_PROJECT_HINT` env var — explicit override for tests and
   deliberately scripted shells.
...
Berth also emits a WezTerm/iTerm2 user variable (OSC 1337
`SetUserVar`) as metadata, but the shell hook does not auto-enter from
that value alone. Fresh terminal windows can inherit user variables
without being opened from a Berth workspace.

Older versions wrote `$XDG_STATE_HOME/berth/last-active` as a fallback.
Current hook code intentionally ignores it because it made unrelated
fresh terminal windows replay the most recent Berth workspace.
