...
#[cfg(windows)]
pub fn ssh_through_filter(args: &[String]) -> anyhow::Result<i32> {
...
mod unix {
    ...
    use crate::ssh::osc7_filter::Osc7Filter;
    ...
    mod tests {
        ...
        fn stdin_forwarder_drains_pipe_before_hup() {
            ...
            let mut out = Vec::new();
            forward_stdin_until_shutdown(read_fd, &mut out, &shutdown, None)
                .expect("forward stdin");
...
#[cfg(unix)]
pub use unix::ssh_through_filter;
